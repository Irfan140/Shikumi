...
type PendingPerm = {
  ...
  input: unknown;
  mode: Mode;
  resolve: (v: "allow" | "deny" | "always") => void;
};
...
export function AppUI({
      ...
      setItems((a) =>
        a.map((x) =>
          x.id === did
            ? ({
                ...x,
                text: (x as unknown as { text: string }).text + extraDelta,
              } as Item)
            : x,
        ),
      );
    ...
    setItems((a) =>
      a.map((x) =>
        x.id === did
          ? ({
              ...x,
              text: (x as unknown as { text: string }).text + delta,
            } as Item)
          : x,
      ),
    );
  ...
  useEffect(() => {
    (
      app.agentLoop as unknown as { opts: Record<string, unknown> }
    ).opts.getMode = () => modeRef.current;
    (
      app.agentLoop as unknown as { opts: Record<string, unknown> }
    ).opts.requestPermission = async (
      toolName: string,
      toolCallId: string,
      toolInput: unknown,
      m: string,
      ...
      for await (const ev of app.agentLoop.run(sid, value, { mode })) {
        switch (ev.type) {
          ...
          case "tool.started":
            ...
            break;
          case "tool.completed":
            update(ev.toolCallId, {
              status: "done",
              detail: String(ev.result).slice(0, 600),
            });
            break;
          case "tool.failed":
            update(ev.toolCallId, {
              status: "error",
              detail: ev.error.slice(0, 600),
            });
            break;
          case "permission.requested":
            push({
              id: uid(),
              kind: "system",
              text: `⚑ permission needed: ${ev.toolName} ${JSON.stringify(ev.input).slice(0, 200)} in ${ev.mode}`,
            });
            break;
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      ...
      {modeToast ? (
        <Box marginTop={1}>
          <Text color={mode === "plan" ? "yellow" : "green"} bold>
            {" "}
            ── {modeToast} ──
          </Text>
        </Box>
      ...
      {scrollOffset > 0 ? (
        <Box marginTop={1}>
          <Text color="yellow" dimColor>
            {" "}
            ── ▲ scrolled up {scrollOffset} · ↓/PgDn to bottom · new messages
            hidden ──
          </Text>
        </Box>
      ) : null}
      <Box flexDirection="column" marginTop={1}>
        {visible.map((it) => {
          if (it.kind === "system")
            return (
              <Text key={it.id} color="gray" dimColor>
                ─ {it.text}
              </Text>
            );
          if (it.kind === "user")
          ...
          if (it.kind === "agent")
            return (
              <Box
                ...
                flexDirection="column"
                borderStyle="round"
                borderColor={it.streaming ? "yellow" : "green"}
                paddingX={1}
              >
                <Box gap={1}>
                  {it.streaming ? (
                  ...
                  <Text bold color={it.streaming ? "yellow" : "green"}>
                    {it.streaming ? "Shikumi is thinking…" : "Shikumi"}
                  </Text>
                </Box>
                {it.text ? (
                  <Text color="white">{it.text}</Text>
          ...
          if (it.kind === "tool") {
            ...
            return (
              <Box key={it.id} flexDirection="column" paddingLeft={2}>
                ...
                {it.detail ? (
                  <Text color="gray">{it.detail.slice(0, 700)}</Text>
                ) : null}
              </Box>
            );
          }
      ...
      {pendingPerm ? (
        ...
        >
          <Text bold color="yellow">
            ⚑ Permission required ({pendingPerm.mode})
          </Text>
          <Text color="white">
            Allow{" "}
            <Text bold color="yellow">
          ...
          <Text color="gray" dimColor>
            {JSON.stringify(pendingPerm.input).slice(0, 300)}
          </Text>
          <Text color="yellow">
            {" "}
            y = allow once · n/Esc = deny · a = always allow{" "}
            {pendingPerm.toolName} · Tab switch mode & allow
          </Text>
        </Box>
      ) : null}
      {slashOpen ? (
      ...
      <Box
        ...
        borderStyle="single"
        borderColor={
          pendingPerm ? "yellow" : mode === "plan" ? "yellow" : "green"
        }
        paddingX={1}
        justifyContent="space-between"
      >
        <Box flexGrow={1} gap={1}>
          <Text color={pendingPerm ? "yellow" : busy ? "yellow" : "cyan"} bold>
            {pendingPerm ? "⚑ " : busy ? "● " : "› "}
          </Text>
          <Text color={input ? "white" : "gray"}>
            {pendingPerm
              ? `Waiting for ${pendingPerm.toolName}… (y/n/a)`
              : input || (busy ? "working — please wait…" : "Ask something…")}
          </Text>
          {!busy && !pendingPerm ? (
