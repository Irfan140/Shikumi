...
export async function runPrompt(prompt: string, sessionId?: string) {
  ...
  if (Object.keys(app.config.mcpServers).length) await app.mcpManager.connectAll();
  for await (const ev of app.agentLoop.run(sid, prompt)) {
    ...
    else if (ev.type === "tool.failed") console.log(`\n✗ ${ev.toolName}: ${ev.error}`);
...
export async function listSessions() {
  ...
  else for (const s of sessions) console.log(`${s.id}  ${s.updatedAt}  ${s.workspaceRoot}`);
  await app.shutdown();
}
