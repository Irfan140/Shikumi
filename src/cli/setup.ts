...
export async function runSetup() {
  ...
  const apiKey = await prompt("OPENAI_API_KEY (sk-...), leave empty to keep Mock: ");
  ...
  const modelCfg: Record<string, unknown> = { ...(existing["model"] as Record<string, unknown> ?? {}) };
  ...
  if (!apiKey) console.log("No key set — running with MockProvider. Add key later via `shikumi setup` or `shikumi config set model.apiKey <key>`");
}
