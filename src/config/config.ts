...
export function loadConfig(workspaceRoot: string = process.cwd()): AppConfig {
  ...
  if (process.env.SHIKUMI_MODEL || process.env.HARNESS_MODEL) {
    envOverrides.model = {
      ...(envOverrides.model as object),
      name: (process.env.SHIKUMI_MODEL ?? process.env.HARNESS_MODEL) as string,
    };
  }
  ...
  if (!cfg.model.apiKey && process.env.OPENAI_API_KEY)
    cfg.model.apiKey = process.env.OPENAI_API_KEY;
