    ...
    if (opts.apiKey) {
      ...
      if (isTracingEnabled()) {
        this.wrapPromise = import("langsmith/wrappers/openai")
          .then((m) => {
            const wrap = (m as unknown as { wrapOpenAI: (c: OpenAI) => OpenAI }).wrapOpenAI;
  ...
  async *stream(request: ModelRequest): AsyncIterable<ModelStreamEvent> {
    ...
    if (!this.client) {
      ...
      for (const tc of r.toolCalls) {
        yield { type: "tool_call.delta", toolCallId: tc.id, nameDelta: tc.name, argsDelta: tc.arguments };
      }
      ...
      for await (const chunk of stream) {
        ...
        if (delta.tool_calls) {
          for (const tc of delta.tool_calls) {
            ...
            yield { type: "tool_call.delta", toolCallId: key, nameDelta: tc.function?.name, argsDelta: tc.function?.arguments };
          }
        }
      }
...
function toOpenAIMessages(req: ModelRequest): OpenAI.Chat.Completions.ChatCompletionMessageParam[] {
...
async function mockGenerate(request: ModelRequest): Promise<ModelResponse> {
  ...
  if (last?.role === "tool") {
    return { content: `Mock: saw tool result "${last.content.slice(0, 200)}". Set OPENAI_API_KEY for real LLM.`, toolCalls: [], finishReason: "stop" };
  }
  const lastUser = [...request.messages].reverse().find((m) => m.role === "user")?.content ?? "";
  const hasTools = request.tools.length > 0;
  if (hasTools && /list|files|directory/i.test(lastUser) && request.tools.find((t) => t.name === "list_directory")) {
    return { content: "", toolCalls: [{ id: "mock_1", name: "list_directory", arguments: JSON.stringify({ path: "." }) }], finishReason: "tool_calls" };
  }
  return { content: `Mock response (no OPENAI_API_KEY set). You said: "${lastUser.slice(0, 200)}". Set OPENAI_API_KEY to enable real model calls.`, toolCalls: [], finishReason: "stop" };
}
...
export class MockProvider implements ModelProvider {
  ...
  async *stream(req: ModelRequest): AsyncIterable<ModelStreamEvent> {
    ...
    if (r.content) yield { type: "text.delta", delta: r.content };
    for (const tc of r.toolCalls) yield { type: "tool_call.delta", toolCallId: tc.id, nameDelta: tc.name, argsDelta: tc.arguments };
    yield { type: "done", response: r };
  }
}
