...
export class AgentLoop {
    ...
    if (isTracingEnabled()) {
      try {
        ...
        const traced = (
          ...
          metadata: {
            sessionId,
            mode: runOpts?.mode ?? this.opts.getMode?.() ?? "agent",
          },
        });
        yield* (traced as unknown as typeof this._run)(
          sessionId,
          userInput,
          runOpts,
        );
  ...
  private async *_run(
    ...
    await this.opts.contextManager.append(sessionId, runId, {
      role: "user",
      content: userInput,
    });
    ...
    try {
      while (iterations < maxIter) {
        ...
        const tools =
          mode === "plan"
            ? ((
                this.opts.toolRegistry as unknown as {
                  definitionsForMode: (m: string) => unknown[];
                }
              ).definitionsForMode?.(mode) ??
              this.opts.toolRegistry.definitions())
            : this.opts.toolRegistry.definitions();
        const stream = this.opts.modelProvider.stream({
          ...
          systemPrompt:
            this.opts.contextManager.getSystemPrompt() +
            (mode === "plan"
  ...
  private async persist(sessionId: string, runId: string, msg: ModelMessage) {
    if (this.opts.messageRepo)
      await this.opts.messageRepo.append(sessionId, runId, msg);
  }
}
