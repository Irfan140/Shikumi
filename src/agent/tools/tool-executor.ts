  ...
  async execute(toolName: string, rawInput: unknown, context: ToolContext): Promise<ToolResult> {
    if (isTracingEnabled()) {
      try {
        ...
        const traced = (traceable as unknown as (fn: typeof this._execute, cfg: unknown) => typeof this._execute)(this._execute.bind(this), {
          name: `tool:${toolName}`,
          run_type: "tool",
          metadata: { toolName, sessionId: context.sessionId, runId: context.runId },
        });
        return await (traced as unknown as (a: string, b: unknown, c: ToolContext) => Promise<ToolResult>)(toolName, rawInput, context);
      } catch {}
    }
    return this._execute(toolName, rawInput, context);
  }

  private async _execute(toolName: string, rawInput: unknown, context: ToolContext): Promise<ToolResult> {
    ...
    if (typeof rawInput === "string") {
      try {
        ...
        throw new ToolValidationError(`Invalid JSON for tool ${toolName}: ${rawInput}`);
      }
    }
    const result = tool.inputSchema.safeParse(parsed);
    if (!result.success) throw new ToolValidationError(`Validation failed for ${toolName}: ${result.error.message}`, result.error);
    try {
